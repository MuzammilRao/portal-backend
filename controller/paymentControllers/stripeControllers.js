...
const WC_STRIPE = require('stripe')(config.WEB_CRAFTPROS_STRIPE_SECRET_KEY);
...
exports.createPaymentIntentWC = CatchAsync(async (req, res, next) => {
  try {
    // IP validation
    const ip = req.headers['x-forwarded-for'] || req.connection.remoteAddress || req.ip;
    const geo = geoip.lookup(ip);
    ...
    let customer;
    try {
      const customers = await WC_STRIPE.customers.list({
         email: req.body.email,
        limit: 1,
      });
      ...
      if (customers.data.length > 0) {
        ...
        // Update customer information if they already exist
        customer = await WC_STRIPE.customers.update(customer.id, {
          name: firstName,
          address: {
            line1: address,
            city: city,
        ...
        // Create new customer if not found
        customer = await WC_STRIPE.customers.create({
          name: firstName,
          email: req.body.email,
          address: {
            line1: address,
    ...
    let ephemeralKey;
    try {
      ephemeralKey = await WC_STRIPE.ephemeralKeys.create(
        { customer: customer.id },
        { apiVersion: '2022-08-01' },
      );
    } catch (error) {
    ...
    let paymentIntent;
    try {
      // CRITICAL FIX: Use WC_STRIPE instead of WIZ_PUB
      paymentIntent = await WC_STRIPE.paymentIntents.create({
        amount: Math.round(parseFloat(req.body.amount) * 100), // Use Math.round for better precision
        currency: req.body.currencyCode,
        customer: customer.id,
        automatic_payment_methods: { enabled: true },
